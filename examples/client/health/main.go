...
var (
	// 定义命令行选项
	addr  = flag.String("addr", "127.0.0.1:6666", "The grpc server address to connect to.") // gRPC 服务的地址
...
func main() {
	...
	// 创建 MiniBlog 客户端
	client := apiv1.NewMineHubClient(conn) // 使用连接创建一个 MiniBlog 的 gRPC 客户端实例
	...
	resp, err := client.Healthz(ctx, &emptypb.Empty{}) // 发起 gRPC 请求，Healthz 是一个简单的健康检查方法
	if err != nil {
